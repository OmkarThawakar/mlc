...
class MultilabelClassifier(nn.Module):
    ...
    def forward(self, img_features):
        ...
        x = self.classifier(img_features)
        x = torch.sigmoid(x)

        return x


model = MultilabelClassifier(inp_dims=512, n_classes=56)
...
def evaluate(model, loss_func, test_loader):
    ...
    for i, data in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            recipe_image_encodings = torch.tensor(recipe_image_encodings, dtype=torch.float, device=device)

            outputs = model(recipe_image_encodings)
...
for epoch in range(EPOCHS):
    ...
    if acc > max_acc :
        ...
        torch.save(state, 'vision/best_ckpt.pth')
...
torch.save(state, 'vision/epoch_{}_ckpt.pth'.format(epoch))
