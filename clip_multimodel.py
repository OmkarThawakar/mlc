...
model = MultilabelClassifier(inp_dims=512*3, n_classes=56)
...
def evaluate(model, loss_func, test_loader):
    ...
    for i, data in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            outputs = model(recipe_image_encodings, ingrs_encodings, instrs_encodings)
...
for epoch in range(EPOCHS):
    ...
    if acc > max_acc :
        ...
        torch.save(state, 'multimodel/best_ckpt.pth')
...
torch.save(state, 'multimodel/epoch_{}_ckpt.pth'.format(epoch))
